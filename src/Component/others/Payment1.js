...
const Payment = ({ onPaymentSelect }) => {
  ...
  if (
    ...
    (!cardDetails.cardNumber || !cardDetails.nameOnCard || !cardDetails.expiry || !cardDetails.cvv)
  ) {
    Swal.fire({
      icon: "warning",
      title: "Missing Info",
  ...
  try {
    ...
    console.log("📦 Writing full order to Firestore...");
    ...
    const orderDoc = {
      ...
      items: cleanedCart,
      totalAmount: cleanedCart.reduce((sum, item) => sum + item.price * item.quantity, 0)
    };
